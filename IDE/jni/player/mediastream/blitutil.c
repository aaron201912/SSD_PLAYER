...
void extractSquareClip_NxN(RECT_t src, RECT_t **clipA, int *clipN, int N)
{
    ...
    *clipA = (RECT_t *)calloc(sizeof(RECT_t), clipNum);
    size_t n = 0;
    clip = *clipA;
...
void extractSquareClip_Even(RECT_t src, RECT_t **clipA, int *clipN)
{
    int clipNum = 0;
    RECT_t *clip = NULL;
    ...
    else
    {
        ...
        *clipA = (RECT_t *)calloc(sizeof(RECT_t), clipNum);
        clip = *clipA;
        memcpy(clip, &src, sizeof(RECT_t));
    }

    *clipN = clipNum;
}
void extractSquareClip_MinArea(RECT_t src, RECT_t **clipA, int *clipN)
{
    int clipNum = 0;
    RECT_t *clip = NULL;
    int i = 0;
    ...
    if (!last_try)
    {
        ...
        clip = (RECT_t *)calloc(sizeof(RECT_t), clipNum - 1 + clipnum0);
        memcpy(clip, *clipA, (clipNum - 1) * sizeof(RECT_t));
        memcpy(clip + clipNum - 1, srcClip0, clipnum0 * sizeof(RECT_t));
        free(srcClip0);
        free(*clipA);
        *clipA = clip;
...
void round_rect(int depth, RECT_t *update_rect)
{
    if (depth == 1)
    {
...
void _get_dst_rect_cw(RECT_t *dst_rect, const RECT_t *src_rect, Surface_t *pDstSurface)
{
...
void _get_src_rect_cw(const RECT_t *dst_rect, RECT_t *src_rect, Surface_t *pDstSurface)
{
...
{
    RECT_t src_update = *(RECT_t *)pRect;
    RECT_t dst_update;
...
void _get_dst_rect_ccw(RECT_t *dst_rect, const RECT_t *src_rect, Surface_t *pDstSurface)
{
...
void _get_src_rect_ccw(const RECT_t *dst_rect, RECT_t *src_rect, Surface_t *pDstSurface)
{
...
void _get_dst_rect_hvflip(RECT_t *dst_rect, const RECT_t *src_rect, Surface_t *pDstSurface)
{
...
void _get_src_rect_hvflip(const RECT_t *dst_rect, RECT_t *src_rect, Surface_t *pDstSurface)
{
